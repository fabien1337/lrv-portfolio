...
<div class="content">
    <h2 class="content-heading">
        ...
        Inbox (<span class="total-mails">{{$nb_mails}}</span>)
    </h2>
    <div class="row">
                ...
                <div class="block">
                    ...
                    <div class="block-content">
                        <ul class="nav nav-pills flex-column push">
                            <li class="nav-item">
                                <a class="nav-link d-flex align-items-center justify-content-between active" href="{{route('admin.contacts.index')}}">
                                    <span><i class="fa fa-fw fa-inbox mr-5"></i> Inbox</span>
                                    <span class="badge badge-pill badge-secondary total-mails">{{$nb_mails}}</span>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link d-flex align-items-center justify-content-between" href="{{route('admin.contacts.trash')}}">
            ...
            <div class="block">
                <div class="block-header block-header-default">
                    <div class="block-title">
                        <strong id="nb-mails">1-10</strong> from <strong class="total-mails">{{$nb_mails}}</strong>
                    </div>
                    <div class="block-options">
                        ...
                        {{-- <button id="refresh-btn" type="button" class="btn-block-option" data-toggle="block-option" data-action="state_toggle" data-action-mode="demo">
                ...
                <div class="block-content">
                    <div class="push">
                        <button type="button" id="delete-btn" class="btn btn-rounded btn-alt-secondary">
...
<script>
    ...
    function getContactPerPage(page) {
        var url = "{{route('api.contact.all')}}?page="+page;
        fetch(url, headers)
    ...
    // Get deleted Ids in array to delete
    var deletedIds = [];
    $(document).on('click', '.css-control-input', (e) => {
        var $checkbox = $(e.target);
        var value = $checkbox.val();
        if (!$checkbox.is(':checked')) {
            deletedIds = deletedIds.filter(item => item !== value)
        }
        else {
    ...
    $('#delete-btn').on('click', () => {
        if (deletedIds.length > 0) {
            fetch("{{route('api.contact.delete')}}", {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'Accept': 'application/json'
